...
2. **§4.5 batch constants** — S3 sinks: batch timeout **5 s**, max batch size
   **10 MB**. Inter-aggregator HTTP sinks: batch timeout **1 s**. No tuning
   ...
   unchanged.
3. **Instance types/counts** — `m6i.large` (generators, Cribl leader),
   `m6i.xlarge` (all aggregator/worker tiers), fixed counts per `PLAN.md` §4.
...
| 2 | Request-concurrency **mechanism** (adaptive algorithm vs. static cap) | Cribl (both `cribl-edge` and `cribl-leader` t1→t2) | Vector's `request.concurrency: adaptive` is a live-adaptive algorithm that scales up/down with observed latency/throughput. Cribl's Webhook destination "Request concurrency" is a fixed integer ceiling (raised from its default 5 to the documented max 32 in this profile). Both are "concurrency allowed to scale" in the PLAN §4.6.2 sense (raised toward maximum vs. left at a low default), but the underlying mechanism differs — recorded as an asymmetry, not a gap. |
...
| Cribl leader (T1→T2 group) | Webhook `compress` | ON (gzip) | `none` — **active change** — `ansible/roles/cribl-leader/templates/groups/outputs.yml.j2:55` | Same active-change rationale as the Cribl Edge row above, applied to the T1→T2 inter-tier hop. | **[Unverified]** https://docs.cribl.io/stream/destinations-webhook/ |
| Cribl leader (T1→T2 group) | Webhook "Request concurrency" (`concurrency` — **TODO(verify)**) | `5` (range 1–32) | `32` — `ansible/roles/cribl-leader/templates/groups/outputs.yml.j2:64` (value from `cribl_t2_webhook_concurrency`, `ansible/roles/cribl-leader/defaults/main.yml`) | Parity counterpart to Vector's adaptive concurrency on the T1→T2 hop. | **[Unverified]** https://docs.cribl.io/stream/destinations-webhook/ |
| Cribl leader (T1→T2 group) | Webhook "Keep alive" | ON (up to 120 s) | ON (kept at default; not set explicitly) — `ansible/roles/cribl-leader/templates/groups/outputs.yml.j2:66-70` (comment) | Same rationale as the Cribl Edge row above. | **[Unverified]** https://docs.cribl.io/stream/destinations-webhook/ |
...
| Cribl leader (t1/t2 groups) | Webhook Persistent Queue / Backpressure behavior (T1→T2) | OFF / `Block` | OFF / `Block` (unchanged) — `ansible/roles/cribl-leader/templates/groups/outputs.yml.j2:71-75` (comment) | Same rationale as the Cribl Edge row above, applied to the T1→T2 hop. | **[Unverified]** Same citations. |
| Cribl leader (t1/t2 groups) | S3 destination (`final_s3`) local file staging | n/a | n/a — no separate buffer/PQ toggle applies | Same note as the Cribl Edge S3 row above, for both groups' terminal S3 sinks. | N/A |
...
| Cribl leader (t1 group, S3/S4) | S3 source "Poll timeout (secs)" (`pollTimeoutSecs` — **TODO(verify)**, see §6) | `10` s (min 1, max 20) | `20` s — `ansible/roles/cribl-leader/templates/groups/inputs.yml.j2:45` (value from `cribl_s3_poll_timeout_secs`, `ansible/roles/cribl-leader/defaults/main.yml`) | 20 s is the documented maximum for this field, matching AWS SQS's own `WaitTimeSeconds` ceiling and `PLAN.md` §4.6.4's literal target. Parity counterpart to Vector's `poll_secs: 20`. | **[Unverified]** No vendor-quantified figure for the 10→20 s change found. https://docs.cribl.io/stream/sources-s3/ |
...
| Vector aggregator | `--threads` / `VECTOR_THREADS` | all available cores | unchanged (no flag/env var set) — `ansible/roles/vector-aggregator/templates/vector.service.j2` (comment added) | Same as the agent row — the `m6i.xlarge` aggregator/worker nodes' 4 vCPUs are already fully used by Vector's default scheduling. | **[Unverified]** Same citation. |
| Cribl leader (both groups) | Group Settings → Worker Processes (`workerProcesses` — **TODO(verify)**, see §6) | `-2` (vCPU count − 2, per Group Settings UI prose) OR `1` (per `cribl.yml` `workers.count` schema comment — **discrepancy in Cribl's own docs, not reconciled**) | `4` (= vCPU count on `m6i.xlarge`) — `ansible/roles/cribl-leader/templates/groups.yml.j2:52` (value from `cribl_worker_processes`, `ansible/roles/cribl-leader/defaults/main.yml`) | A positive integer sets an EXACT worker-process count (`docs.cribl.io/stream/scaling/`: "Positive numbers specify an absolute number of Workers"); 4 = one worker process per vCPU on `m6i.xlarge`, maximizing parallelism per `PLAN.md` §4.6.5's "Cribl worker processes set to vCPU count." | **[Unverified]** Vendor doc states a sizing rule of thumb (~400 GB/day per physical core, per Cribl's scaling guidance) but no direct before/after latency figure for changing the worker-process count from -2/1 to 4 on this exact workload. https://docs.cribl.io/stream/scaling/ |
...
$ grep -n "cribl_edge_s3_flush_secs\|cribl_edge_s3_max_bytes\|cribl_s3_flush_secs\|cribl_s3_max_bytes_mb" \
    ansible/roles/cribl-edge/defaults/main.yml ansible/roles/cribl-leader/defaults/main.yml
...
ansible/roles/cribl-leader/defaults/main.yml:52:cribl_s3_flush_secs: 5              # ->S3: 5 s  (== Vector timeout_secs)
ansible/roles/cribl-leader/defaults/main.yml:53:cribl_s3_max_bytes_mb: 10          # 10 MB     (== Vector max_bytes)
...
$ grep -n "vector_http_batch_timeout_secs\|cribl_http_flush_secs" \
    ansible/roles/vector-aggregator/defaults/main.yml ansible/roles/cribl-leader/defaults/main.yml
ansible/roles/vector-aggregator/defaults/main.yml:44:vector_http_batch_timeout_secs: 1         # inter-aggregator HTTP: 1 s (PLAN §4.5)
ansible/roles/cribl-leader/defaults/main.yml:54:cribl_http_flush_secs: 1           # inter-tier HTTP: 1 s (== Vector http batch)
```

All values match `PLAN.md` §4.5 exactly (5 s / 10 MB for S3, 1 s for
inter-tier HTTP) and match the pre-tuning values recorded before any §4.6
edits began.
...
| 2 | Webhook "Request concurrency" | `concurrency` | `ansible/roles/cribl-edge/templates/outputs.yml.j2`, `ansible/roles/cribl-leader/templates/groups/outputs.yml.j2` | Compare against `$CRIBL_HOME/local/cribl/outputs.yml` (Edge) or `groups/<g>/local/cribl/outputs.yml` (leader-managed) after setting via UI. |
...
| 4 | S3 source "Poll timeout (secs)" | `pollTimeoutSecs` | `ansible/roles/cribl-leader/templates/groups/inputs.yml.j2` | Compare against `groups/<g>/local/cribl/inputs.yml` for the S3 source type. |
| 5 | Group Settings "Worker Processes" | `workerProcesses` | `ansible/roles/cribl-leader/templates/groups.yml.j2` | Compare against `$CRIBL_HOME/local/cribl/groups.yml` after setting via UI. Note the unreconciled discrepancy between the Group Settings UI's documented default (`-2`) and the node-level `cribl.yml` `workers.count` schema comment's documented default (`1`) — confirm which mechanism this key path actually maps to (per-group leader-pushed override vs. node-level `cribl.yml`) before relying on this row for the analysis phase. |
