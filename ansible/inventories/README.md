...
(generator hosts + agents) and a **logging** account (aggregators — Vector
tiers + standalone Cribl Stream nodes, no leader — buckets). The `aws_ec2`
plugin's `aws_profile` option is **`type: str`** — it queries exactly **one**
account per inventory source. We therefore use **two inventory config
files** in this directory, one per profile:

| File | Account | Populates groups |
|------|---------|------------------|
| `sender.aws_ec2.yml`  | sender  | `role_generator`, agent stacks, both OS |
| `logging.aws_ec2.yml` | logging | `role_agg_t1/t2`, Cribl stack |
